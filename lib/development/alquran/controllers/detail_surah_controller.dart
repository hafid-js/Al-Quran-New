...
import 'package:alquran_new/development/alquran/data/local/datasource/surah_local_datasource.dart';
import 'package:alquran_new/development/alquran/data/local/datasource/tafsir_local_datasource.dart';
...
import 'package:alquran_new/development/alquran/domain/entities/detail_surah.dart';
...
class DetailSurahController extends GetxController {
  ...
  Future<void> _saveDetailSurahToCache(DetailSurah data) async {
    final existing = DevHiveService.surahBox.values
    ...
    if (existing.isNotEmpty) {
      ...
      surah = SurahCache()
        ...
        ..audioUrl = data.audioFull.values.isNotEmpty
            ? data.audioFull.values.first
            : '';
      await DevHiveService.surahBox.add(surah);
    }

    final ayatToDelete = DevHiveService.ayatBox.values
        .where((a) => a.surahNomor == data.nomor)
        .map((a) => a.key as int)
        .toList();
    for (final key in ayatToDelete) {
      await DevHiveService.ayatBox.delete(key);
    }
    ...
    for (final a in data.ayat) {
      ...
      await DevHiveService.ayatBox.add(ayat);
    }
  }

  void _displayFromCache(SurahCache cache) {
    final sortedAyat =
        DevHiveService.ayatBox.values
  ...
  Future<void> debugPrintSurahByNomor(int nomor) async {
    ...
    final data = DevHiveService.surahBox.values
  ...
  Future<void> clearAllCache() async {
    ...
    await DevHiveService.ayatBox.clear();
  }
    ...
    try {
      ...
      if (cache.isNotEmpty) {
        tafsirList.assignAll(
          cache
              .map((e) => TafsirAyat(ayat: e.nomorAyat, teks: e.tafsir))
              .toList(),
        );
      }
  ...
  Future<void> playAudio(Ayat ayat) async {
    ...
    try {
      ...
      final qariKey = (setting.qariSelected.value + 1).toString().padLeft(
        2,
        '0',
      );
